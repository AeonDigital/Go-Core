...
const xerror_CTX string = "XCONFIG"
...
func (o *Options) ValidateExclusivePaths(parserType string) error {
	...
	if count > 1 {
		return xerrors.NewErr(
			...
			"",
			"",
			xerrors.MsgData("FilePath", o.FilePath, "DirPath", o.DirPath, "ConfigPath", o.ConfigPath),
			xerrors.MsgOptions("FilePath", "DirPath", "ConfigPath"),
		)
	}
...
func (o Options) RetrieveConfigFilePaths(extensions []string, parserType string) ([]string, error) {
	...
	if len(configFiles) == 0 {
		return nil, xerrors.NewErr(
			...
			"",
			"configFiles",
			xerrors.MsgData("ConfigPath", o.ConfigPath, "DirPath", o.DirPath, "FilePath", o.FilePath),
		)
	}
