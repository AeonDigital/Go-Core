...
func TestParser_Read_SuccessAndMerge(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(tmpDir, "01_base.json"), []byte(json1), 0644); err != nil {
		t.Fatalf("failed to setup test json file 1: %v", err)
	}
	if err := os.WriteFile(filepath.Join(tmpDir, "02_override.json"), []byte(json2), 0644); err != nil {
...
func TestParser_Read_EmptyFilesAndErrors(t *testing.T) {
	...
	if err := os.WriteFile(badFilePath, []byte("{bad-json-syntax}"), 0644); err != nil {
	...
	if !strings.Contains(errBad.Error(), "CTX: PARSER.JSON") {
		t.Errorf("unexpected error payload message: %q", errBad.Error())
	}
}

func TestParser_Read_FileSystemFailure(t *testing.T) {
	// Cenário: Erro de leitura de infraestrutura (RetrieveConfigFilePaths falha por diretório inexistente)
	p := json.NewParser()
	opts := xconfig.Options{
	...
	if !strings.Contains(err.Error(), "CTX: XCONFIG") {
		t.Errorf("expected corporate error prefix tracking, got: %q", err.Error())
	}
}
...
func TestParser_Read_ReadFilePermissionFailure(t *testing.T) {
	...
	if err := os.WriteFile(filePath, []byte(`{"secure": true}`), 0644); err != nil {
		t.Fatalf("failed to setup temporary json file: %v", err)
	}

	// 2. Remove todas as permissões de leitura/escrita do arquivo (0000)
	// Nota: Em ambientes Windows ou rodando testes como 'root/sudo' no Linux,
	// o Chmod 0000 pode ainda permitir leitura. Se o seu ambiente ignorar o Chmod,
	// o plano B infalível é deletar o arquivo do disco usando 'os.Remove(filePath)'
	// imediatamente ANTES da chamada do p.Read().
	...
	errStr := err.Error()
	// Technical Note: Validates the structured layout and metadata blocks generated by buildMask
	if !strings.Contains(errStr, "[CTX: PARSER.JSON]") {
		t.Errorf("expected context block 'PARSER.JSON', got: %q", errStr)
	}
	if !strings.Contains(errStr, "[MSG: target resource not found]") {
		t.Errorf("expected default resource message block, got: %q", errStr)
	}
	if !strings.Contains(errStr, "[FIELD: filePath]") {
		t.Errorf("expected field targeting parameter to be 'filePath', got: %q", errStr)
	}
